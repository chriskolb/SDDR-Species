...
train <- full.list$basic.dfs$df.tune
...
#min.unique <- 25
if (pred.type == "deep"){min.unique <- 20} else {min.unique <- 45}

# run data pre-processing script
source(file = file.path(main.path, "auxiliary-scripts", "data-preprocessing.R"))

if (pred.type != "deep"){
  x.train.s <- cbind(x.train.s, spec = train.specs)
  x.test.s <- cbind(x.test.s, spec = test.specs)
  }
...
# model formula
if (pred.type == "smooth"){
  form.mod <- form.smooth.by
} else if (pred.type == "smooth-deep"){
  form.mod <- form.smooth.deep.by
} else{
  form.mod <- form.deep
}
form.mod
...
scoringFunction <- function(hidden.units.num = 3, hidden.layers = 3, log.lr.nn = -3, log.decay.nn = -4, 
  ...
  for(fold in 1:4){
    ...
    cat("Validation AUC for pooled model in this fold is = ", auc.bayes.opt.run, "\n")
...
ScoreResult <- bayesOpt(
  ...
  , saveFile = file.path(temp.data, paste0("bayes-opt-pooled-", pred.type, "-",  timestamp, ".rds"))
)
...
#ScoreResultAdd <- readRDS(file.path())
...
for(spec in insample.species){
  ...
  train <- eval(parse(text = paste0("full.list$train.dfs.spec$train.full.", gsub(" ", "", spec, fixed = TRUE))))
  ...
  cat(" Final univariate pooled model for species", spec, "and predictor", pred.type, "\n",  
...
# save results
saveRDS(simu.runs.all, file = file.path(temp.data, paste0("simu-runs-pooled-", pred.type, "-",timestamp, ".rds")))
saveRDS(brier.runs.all, file = file.path(temp.data, paste0("brier-simus-pooled-", pred.type, "-",timestamp, ".rds")))
